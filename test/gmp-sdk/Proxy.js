...
const FinalProxy = require('../../artifacts/contracts/gmp-sdk/upgradable/FinalProxy.sol/FinalProxy.json');
const FixedProxy = require('../../artifacts/contracts/gmp-sdk/upgradable/FixedProxy.sol/FixedProxy.json');
...
describe('Proxy', async () => {
    ...
    describe('FixedProxy', async () => {
        ...
        beforeEach(async () => {
            fixedProxyFactory = await ethers.getContractFactory(FixedProxy.abi, FixedProxy.bytecode, owner);
    ...
    describe('Proxy & BaseProxy', async () => {
        ...
        beforeEach(async () => {
            ...
            invalidSetupProxyImplementationFactory = await ethers.getContractFactory('InvalidSetupProxyImplementation', owner);
            ...
            invalidSetupProxyImplementation = await invalidSetupProxyImplementationFactory.deploy().then((d) => d.deployed());
        });
        ...
        it('reverts on invalid implementation', async () => {
            ...
            await expect(
                proxyFactory.deploy(invalidProxyImplementation.address, owner.address, setupParams, { gasLimit: 250000 }),
        ...
        it('reverts if setup params are provided but implementation setup fails', async () => {
            ...
            await expect(
                proxyFactory.deploy(invalidSetupProxyImplementation.address, owner.address, setupParams, { gasLimit: 250000 }),
        ...
        it('implementation returns correct address', async () => {
            ...
            proxy = await proxyFactory.deploy(proxyImplementation.address, owner.address, setupParams).then((d) => d.deployed());
    ...
    describe('FinalProxy', async () => {
        ...
        beforeEach(async () => {
            finalProxyFactory = await ethers.getContractFactory(FinalProxy.abi, FinalProxy.bytecode, owner);
            proxyImplementationFactory = await ethers.getContractFactory('ProxyImplementation', owner);
            differentProxyImplementationFactory = await ethers.getContractFactory('DifferentProxyImplementation', owner);
            invalidSetupProxyImplementationFactory = await ethers.getContractFactory('InvalidSetupProxyImplementation', owner);
        ...
        it('returns false initially from isFinal', async () => {
            ...
            finalProxy = await finalProxyFactory.deploy(proxyImplementation.address, owner.address, setupParams).then((d) => d.deployed());
        ...
        it('reverts if setup params are provided but implementation setup fails', async () => {
            ...
            await expect(finalProxy.finalUpgrade(bytecode, setupParams)).to.be.revertedWithCustomError(finalProxy, 'SetupFailed');
        });
        ...
        it('reverts on second call to finalUpgrade', async () => {
            ...
            await expect(finalProxy.finalUpgrade(bytecode, setupParams)).to.be.revertedWithCustomError(finalProxy, 'AlreadyDeployed');
        });
    });
});
