...
describe('Create3Deployer', () => {
    ...
    describe('deploy', () => {
        it('should deploy to the predicted address', async () => {
            ...
            const contract = await deployCreate3Contract(deployer, userWallet, BurnableMintableCappedERC20, key, [name, symbol, decimals]);
        ...
        it('should deploy the same contract twice to different addresses with different salts', async () => {
            ...
            for (const key of keys) {
                ...
                const contract = await deployCreate3Contract(deployer, userWallet, BurnableMintableCappedERC20, key, [
                    name,
                    symbol,
                    decimals,
                ]);
        ...
        it('should revert if contract is deployed to an address where a contract already exists', async () => {
            ...
            await expect(
                deployCreate3Contract(deployer, userWallet, BurnableMintableCappedERC20, key, [name, symbol, decimals]),
        ...
        it('should revert if a contract that self-destructs is deployed a second time', async () => {
            ...
            await deployCreate3Contract(deployer, userWallet, MockDepositReceiver, key, [receiverWallet.address]);

            // Attempt to deploy MockDepositReceiver again, should fail
            await expect(deployCreate3Contract(deployer, userWallet, MockDepositReceiver, key, [receiverWallet.address])).to.be.reverted;
        });
    });
