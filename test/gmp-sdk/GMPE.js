...
describe('GMPE', async () => {
    ...
    before(async () => {
        ...
        valuedAxelarExpressExecutableTestFactory = await ethers.getContractFactory('AxelarValuedExpressExecutableTest', ownerWallet);
    });
    ...
    beforeEach(async () => {
        ...
        expressExecutableTest = await expressExecutableTestFactory.deploy(destinationChainGateway.address).then((d) => d.deployed());
    ...
    describe('AxelarExpressExecutable', () => {
        it('should expressCallWithToken a swap on remote chain', async () => {
            ...
            const sourceChainTokenA = tokenFactory.connect(userWallet).attach(await sourceChainGateway.tokenAddresses(symbolA));
            await sourceChainTokenA.approve(sourceChainSwapCaller.address, swapAmount);
            await expect(sourceChainSwapCaller.connect(userWallet).swapToken(symbolA, symbolB, swapAmount, userWallet.address.toString()))
            ...
            await expect(
                destinationChainSwapExpress
                    .connect(userWallet)
                    .expressExecuteWithToken(approveCommandId, sourceChain, sourceChainSwapCaller.address, payload, symbolA, swapAmount),
            )
            ...
            const approveExecute = await destinationChainGateway.approveContractCallWithMint(approveWithMintData, approveCommandId);
    ...
    describe('AxelarExpressExecutableTest Execute', () => {
        ...
        async function approve() {
            ...
            await expect(approveExecute)
                ...
                .withArgs(commandId, sourceChain, sourceAddress, contract.address, payloadHash, sourceTxHash, sourceEventIndex);
        }
    ...
    describe('AxelarExpressExecutableTest Execute With Token', () => {
        ...
        async function approve() {
            const approveWithMintData = defaultAbiCoder.encode(
                ['string', 'string', 'address', 'bytes32', 'string', 'uint256', 'bytes32', 'uint256'],
                [sourceChain, sourceAddress, contract.address, payloadHash, tokenSymbol, amount, sourceTxHash, sourceEventIndex],
            );
        ...
        async function expressNotPayed() {
            const expressTx = contract.expressExecuteWithToken(commandId, sourceChain, sourceAddress, payload, tokenSymbol, amount);
        ...
        async function expressSuccess() {
            ...
            let expressTx = contract.expressExecuteWithToken(commandId, sourceChain, sourceAddress, payload, tokenSymbol, amount);
            await expect(expressTx)
                .to.emit(tokenA, 'Transfer')
            ...
            expressTx = contract.expressExecuteWithToken(commandId, sourceChain, sourceAddress, payload, tokenSymbol, amount);
            await expect(expressTx).to.be.revertedWithCustomError(contract, 'ExpressExecutorAlreadySet');
        ...
        async function execution() {
            const executeTx = contract.executeWithToken(commandId, sourceChain, sourceAddress, payload, tokenSymbol, amount);
            await expect(executeTx)
                .to.emit(tokenA, 'Transfer')
    ...
    describe('AxelarValuedExpressExecutableTest Execute With Token (native token)', () => {
        ...
        async function expressSuccess() {
            ...
            expressTx = contract.expressExecuteWithToken(commandId, sourceChain, sourceAddress, payload, tokenSymbol, amount, { value });
            await expect(expressTx).to.be.revertedWithCustomError(contract, 'ExpressExecutorAlreadySet');
